...
import type {
  ISatisfactionRepository,
  SatisfactionStats,
} from "@/src/domain/repositories/ISatisfactionRepository"
...
class SatisfactionRepository implements ISatisfactionRepository {
  ...
  async getStats(): Promise<SatisfactionStats> {
  ...
  async getMonthlyTrend(): Promise<{ month: string; media: number; avaliacoes: number }[]> {
    const all = await this.list()
  ...
  async getDistribution(): Promise<{ nota: string; total: number }[]> {
    const all = await this.list()
